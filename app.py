...
def ui_ambil_barang():
    ...
    def process_scanned_sku():
        ...
        # LOGIKA PERHITUNGAN AKURAT DIKEMBALIKAN:
        # Karena kita membuat kotak baru setiap kali alat scan memicu proses, 
        # maka teks yang menumpuk (contoh: MJ423MJ423) adalah murni hasil scan multi-tembakan yang sangat cepat.
        # Oleh karena itu, kita HARUS menghitungnya secara dinamis!
        jumlah_scan = 1
        ...
        # Increment counter memicu kotak baru yang 100% kosong tanpa kursor lepas
        st.session_state.scan_counter += 1
                    ...
                    for sku_to_reduce, qty_to_reduce in st.session_state.scan_cart.items():
                        ...
                        if item_ditemukan:
                            if qty_to_reduce > item_ditemukan["stok"]:
                                st.session_state.global_notif = {"tab": "ambil", "type": "error", "text": f"❌ Gagal! Stok '{sku_to_reduce}' (Sisa: {item_ditemukan['stok']}) tak cukup."}
                                berhasil = False
                                break
