...
# ==================== CALLBACK TAMBAH RAK (PEMISAH RAHASIA ||| ) ====================
def on_enter_tambah_rak():
    raw_val = st.session_state.get("input_rak_baru_scan", "").strip()
    if not raw_val: return
    
    # Membelah bola salju menggunakan pembatas rahasia ||| buatan Javascript
    items_to_add = [x.strip() for x in raw_val.split("|||") if x.strip()]
...
if st.session_state.mode_aplikasi is None:
    ...
    # --- JAVASCRIPT GLOBAL (KURSOR TAHAN BANTING & PEMISAH RAHASIA) ---
    components.html("""
        <script>
        const doc = window.parent.document;
        const nativeInputValueSetter = Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype, "value").set;

        // Bersihkan event listener lama untuk mencegah bug dobel klik
        ...
        // 1. KONTROL SCANNER BOLA SALJU (SISTEM PEMBATAS RAHASIA ||| )
        doc.gudang_keydown = function(e) {
            if (e.key === 'Enter') {
                ...
                // SISIPKAN PEMBATAS ||| SECARA GAIB DI KOTAK SCAN CEPAT
        ...
        // 2. KONTROL FOKUS KURSOR SAAT TAB DIKLIK (TANPA PAKSAAN TERLEMPAR KE TAB 1)
