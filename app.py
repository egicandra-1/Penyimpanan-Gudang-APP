...
# ==================== CALLBACK TAMBAH RAK (PEMISAH RAHASIA 100% AMAN) ====================
if "tambah_rak_last_raw" not in st.session_state:
    st.session_state.tambah_rak_last_raw = ""

def on_enter_tambah_rak():
    raw_val = st.session_state.get("input_rak_baru_scan", "")
    if not raw_val: return
    
    last_val = st.session_state.tambah_rak_last_raw
    
    # Deteksi bagian teks yang baru saja bertambah
    if last_val and raw_val.startswith(last_val):
        new_part = raw_val[len(last_val):]
    else:
        new_part = raw_val
        
    st.session_state.tambah_rak_last_raw = raw_val
    
    # Belah data menggunakan pemisah rahasia yang disuntikkan Javascript
    items_to_add = [x.strip() for x in new_part.split(" * ") if x.strip()]
    
    added_count = 0
    for clean_val in items_to_add:
    ...
    if added_count > 0:
        save_data_to_sheets()
        pesan = f"✅ {added_count} Rak berhasil ditambahkan!" if added_count > 1 else f"✅ Rak '{items_to_add[0]}' berhasil ditambahkan!"
        st.session_state.global_notif = {"tab": "rak", "type": "success", "text": pesan}
    elif items_to_add:
        st.session_state.global_notif = {"tab": "rak", "type": "error", "text": f"❌ Rak sudah ada terdaftar."}
...
# ==================== CALLBACK TAB AMBIL (PEMISAH RAHASIA) ====================
if "ambil_scan_last_raw" not in st.session_state:
    st.session_state.ambil_scan_last_raw = ""

def process_scanned_sku():
    raw_val = st.session_state.get("quick_scan_input", "")
    if not raw_val: return

    last_val = st.session_state.ambil_scan_last_raw
    if last_val and raw_val.startswith(last_val):
        new_part = raw_val[len(last_val):]
    else:
        new_part = raw_val
        
    st.session_state.ambil_scan_last_raw = raw_val
    ...
    scans_to_process = [x.strip() for x in new_part.split(" * ") if x.strip()]
    daftar_sku_rak = [item["sku"] for item in st.session_state.rak_gudang_tanpa_posisi[rak_terpilih]]
    
    for sku_scan in scans_to_process:
        sku_terdeteksi = None
        for real_sku in daftar_sku_rak:
            if sku_scan.lower().endswith(real_sku.lower()) or real_sku.lower() in sku_scan.lower():
        ...
        if sku_terdeteksi:
            ...
            st.session_state.global_notif = {"tab": "ambil", "type": "error", "text": f"❌ DITOLAK! SKU '{sku_scan}' tidak dikenali di rak '{rak_terpilih}'!"}
...
if st.session_state.mode_aplikasi is None:
    ...
    # --- JAVASCRIPT GLOBAL (KURSOR ANTI GAGAL & INJEKSI PEMISAH RAHASIA) ---
    components.html("""
        <script>
        const doc = window.parent.document;
        const nativeInputValueSetter = Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype, "value").set;

        // Bersihkan memori usang agar tombol tidak bentrok / dobel
        ...
        doc.gudang_keydown = function(e) {
            if (e.key === 'Enter') {
                ...
                // 1. SISIPKAN PEMBATAS BINTANG ( * ) PADA KOTAK SCAN CEPAT 
                // Ini akan mencegah teks bergabung menjadi B-1B-2B-3 saat discan secara membabi buta
                if (text === 'Nama Rak Baru:' || text === 'Scan Kode SKU (Otomatis mendeteksi SKU & menambah jumlah):') {
                    if (!active.value.endsWith(' * ')) {
                        nativeInputValueSetter.call(active, active.value + ' * ');
                        active.dispatchEvent(new Event('input', { bubbles: true }));
                    }
                }

                // 2. KONTROL LOMPAT KURSOR
